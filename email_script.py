...
def send_email(sender_address,sender_name,recipient_address,template_name,tag_list,subject_line):
    ...
    import pg
    conn = pg.DB(host="server", user="databaseuser", passwd="pass", dbname="datbasename")
...
perc
for i in range(0,12):
 send_email("email","The Dawgs", str(x.values[i,1]),"thez_version2",{'PERCENT':'%s%%' % perc,'USER_NAME':str(x.values[i,0])},"Thez- Events Activity Report")
